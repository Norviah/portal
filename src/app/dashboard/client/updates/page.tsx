...
import { GetBoard, GetBoards } from "@/actions/test";
...
import { TweetPost } from "./[slug]/tweet-component";
...
export default function ManagerPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [data, setData] = useState<Awaited<ReturnType<typeof GetBoards>> | undefined>(undefined);
...
function ProjectRadioGroup({ data }: { data: Awaited<ReturnType<typeof GetBoards>> }) {
  const [id, setId] = useState<undefined | string>(undefined);

  return (
    <div>
      <RadioGroup className="p-10 bg-card">
        {data?.map((project) => {
          return (
        ...
        <Link href={`/dashboard/client/updates/${id}`}>
          <Button className="w-md" disabled={id === undefined}>
            See Updates
          </Button>
        </Link>
      </RadioGroup>
